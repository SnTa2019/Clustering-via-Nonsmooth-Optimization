c=============================================================
c IDCA-CLUST: incremental DCA for clustering  
c
c The name of input file should be: datainput
c
c The name of output file is: DCAResults
c
c=============================================================
     ...
     2 ,fval(maxrec),x5(maxsize),plabel(maxclass),abar(maxnft)
     3 ,fval1(maxrec),dcent(maxclust,maxclust)
      integer nk(maxclust,maxrec),nel(maxclust),nob(maxclass)
     1 ,lcand(maxrec)
      common /c22/a,/anclust/nclust,/cnft/nft,/cmf/mf,/cgamma4/gamma4
     1 ,/crecord/nrecord,/cnc/nc,/cnk/nk,nel,/adminim/dminim
     2 ,/ccand/ncand,lcand,/ctnorm/tnorm,/cnob/plabel,nob,/cabar/abar
     3 ,/cnclass/nclass,/cnpnout/npurity,/cgamma/gamma1,gamma2
     4 ,/cdcent/dcent,/cind/ind
      open(40,file='DCAResults.txt')
...
c=======================================================================
   ...
   1  end do
      ...
      subroutine dcaaux(x,fval)
      implicit double precision (a-h,o-z)
      PARAMETER(maxdim=2000, maxrec=200000, maxclust=100, maxnft=100)
      double precision x(maxdim),a(maxrec,maxnft),dminim(maxrec)
     1 ,a1(maxnft) 
      integer list1(maxrec),list2(maxrec),list3(maxrec)
  ...
  1   l1=0
      ...
      do i=1,nrecord
       ...
       tnorm=tnorm+1.0d+00
       if(f1.lt.dminim(i)) then
        l1=l1+1
        list1(l1)=i
       end if
       if(f1.gt.dminim(i)) then
        l2=l2+1
        list2(l2)=i
       end if
       if(f1.eq.dminim(i)) then
        l3=l3+1
        list3(l3)=i
       end if
      end do
c     Stopping criterion
      ...
      d1=abs(d1) 
      if(d1.le.eps) go to 2
      
      do j=1,mf
       x(j)=(dble(l2)*x(j)+a1(j))/dble(nrecord)
      end do 
   ...
   2  fval=0.0d+00
      ...
      subroutine dcaclust(f,x)
      implicit double precision (a-h,o-z)
      PARAMETER(maxdim=2000, maxrec=200000, maxclust=100, maxnft=100)
      double precision x(maxdim),a(maxrec,maxnft),abar(maxnft)
     1 ,ac(maxclust,maxnft),y(maxdim)
  ...
  1   do k=1,nc
      ...
      do i=1,nrecord
       ...
       nmembers(kmin)=nmembers(kmin)+1
       lmembers(kmin,nmembers(kmin))=i
      end do
      ...
      do k=1,nc
       do j=1,mf
        ...
        do l=1,j1
         l1=lmembers(k,l)
         ac(k,j)=ac(k,j)+a(l1,j)
        end do
        if(j1.gt.0) ac(k,j)=ac(k,j)/dble(j1)
       end do
      end do
...
c     Stopping criterion
      ...
      do k=1,nc
       d2=dble(nmembers(k))/dble(nrecord)
       do j=1,mf
        y(j+(k-1)*mf)=(1.0d+00-d2)*x(j+(k-1)*mf)+d2*ac(k,j)   
       end do
      end do
      ...
      do k=1,nc
       do j=1,mf
        d1=d1+abs(y(j+(k-1)*mf)-x(j+(k-1)*mf))
       end do
      end do
      if(d1.le.eps) go to 2
